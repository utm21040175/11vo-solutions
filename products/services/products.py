...
def get_all_products(db: Session, skip: int = 0, limit: int = 10) -> list[ProductResponse]:
    return db.query(Product).offset(skip).limit(limit).all()

def update_product(db: Session, product_id: int, product_data: ProductCreate) -> ProductResponse | None:
